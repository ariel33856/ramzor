...
export default function ContactsArchive() {
  ...
  const [columnOrder, setColumnOrder] = useState(() => {
    const saved = localStorage.getItem('contactsArchiveColumns');
  ...
  React.useEffect(() => {
    if (customFieldsData.length > 0) {
      setColumnOrder(currentColumns => {
        ...
        if (hasChanges) {
          localStorage.setItem('contactsArchiveColumns', JSON.stringify(updatedColumns));
  ...
  const handleDragEnd = (result) => {
    ...
    localStorage.setItem('contactsArchiveColumns', JSON.stringify(items));
  };
  ...
  const toggleColumnVisibility = (columnId) => {
    ...
    localStorage.setItem('contactsArchiveColumns', JSON.stringify(updated));
  };
      ...
      <div className="mx-auto p-1">
        {isLoading ? (
          ...
          >
            ...
            <h3 className="text-xl font-semibold text-gray-600 mb-2">אין תיקים בארכיון</h3>
            <p className="text-gray-400">שנה את הסינון או חזור לדף הראשי</p>
          </motion.div>
        ) : (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100">
            <div className="overflow-x-auto max-h-[75vh]">
