    ...
    <>
        ...
        <div className="flex" ref={tabsRef}>
          {(() => {
            const tabDefs = [
              ...
              { id: 'transactions', label: 'עסקאות', activeBg: 'bg-yellow-50', activeText: 'text-yellow-700', borderColor: 'border-yellow-400', bottomBorderColor: 'border-b-yellow-400' },
            ];
      ...
      <div className="flex items-start gap-2 flex-wrap">
        ...
        {activeTab === 'transactions' && (
          <div className="p-6 bg-yellow-50 border-2 border-yellow-400 rounded-b-lg" style={{ minHeight: '80vh', marginTop: '-2px' }}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-900">עסקאות</h2>
              <Button
                onClick={() => {
                  if (linkedPropertiesData.length === 0) {
                    alert('נא לשייך נכס תחילה');
                    return;
                  }
                  createTransactionMutation.mutate({
                    property_id: linkedPropertiesData[0].id,
                    transaction_type: 'purchase',
                    transaction_date: new Date().toISOString().split('T')[0],
                    amount: null,
                    buyer_name: '',
            ...
            {allTransactions.filter(t => linkedPropertiesData.some(p => p.id === t.property_id)).length === 0 ? (
              ...
              <div className="space-y-4">
                {allTransactions
                  .filter(t => linkedPropertiesData.some(p => p.id === t.property_id))
                  .map((transaction, index) => {
                    ...
                    return (
                      <div key={transaction.id} className="bg-white rounded-xl shadow-sm border-2 border-yellow-200 p-6">
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            <h3 className="text-lg font-bold text-gray-900">
                              עסקה #{index + 1} - {typeLabels[transaction.transaction_type]}
                            </h3>
                            {property && (
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          <div>
                            <Label className="text-sm">סוג עסקה</Label>
                            <Select 
                              value={transaction.transaction_type} 
                              onValueChange={(val) => {
                                updateTransactionMutation.mutate({ 
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-4">
                          <div>
                            <Label className="text-sm">קונה</Label>
                            <Input
                              value={transaction.buyer_name || ''}
                              onChange={(e) => {
                                updateTransactionMutation.mutate({ 
                          ...
                          <div>
                            <Label className="text-sm">מוכר</Label>
                            <Input
                              value={transaction.seller_name || ''}
                              onChange={(e) => {
                                updateTransactionMutation.mutate({ 
