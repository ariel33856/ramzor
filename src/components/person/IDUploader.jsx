...
export default function IDUploader({ onDataExtracted, initialData = null, gender, setGender, compactMode = false }) {
  ...
  const handleFileUpload2 = async (e) => {
    ...
    try {
      ...
      if (!preview && result.document_type === 'appendix') {
        setDetectionResult2('appendix');
        setError('✓ זוהה ספח - נא להשלים תעודת זהות');
        return;
      }

      // Clear error if document was recognized properly
      setError(null);
  ...
  const handleFileUpload3 = async (e) => {
    ...
    try {
      ...
      // Merge data - preserve critical fields from previous documents
      const mergedData = { 
        ...extractedData, 
        ...result,
        // Preserve id_issue_date and id_expiry_date from first document if not in third
        id_issue_date: result.id_issue_date || extractedData.id_issue_date,
        id_expiry_date: result.id_expiry_date || extractedData.id_expiry_date,
        file_url_3: file_url, 
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`grid gap-4 overflow-visible ${
        idType === 'ביומטרית' && detectionResult && detectionResult !== 'both' 
          ? 'grid-cols-3' 
          : 'grid-cols-2'
      }`}>
         {/* Upload Section 1 */}
        ...
        {/* Upload Section 2 - Back Side for biometric or Appendix for regular */}
         {(detectionResult && detectionResult !== 'both') || preview2 ? (
           <>
             {/* Section 2 */}
             <div 
            ...
            {(idType === 'ביומטרית' && preview2) || preview3 ? (
            <div 
      ...
      {!compactMode && (
      <div className={`grid grid-cols-4 md:grid-cols-8 gap-4 p-4 rounded-xl ${extractedData ? 'bg-blue-50 border-2 border-blue-200' : 'bg-gray-50 border-2 border-gray-200'}`}>
        <div>
