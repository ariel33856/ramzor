      ...
      {tab === "reverse" && <div style={{ background:K.cardGrad(), margin:"0 16px", borderRadius:"0 0 12px 12px", border:`1px solid ${K.border}` }}>
        ...
        <div style={{ padding:24, display:"grid", gridTemplateColumns:"1fr 1fr", gap:28 }}>
          <div style={{ display:"grid", gap:14 }}>
            {reverseMode==="pmt2amount"&&[{l:"החזר חודשי רצוי",el:<AmountInputC value={revPmt} onChange={setRevPmt} style={{width:128}}/>},{l:"ריבית %",el:<NI value={revRate} onChange={setRevRate} min={0} max={15} step={0.1} style={{width:88}}/>},{l:"תקופה (שנים)",el:<NI value={revYears} onChange={setRevYears} min={1} max={30} step={1} style={{width:88}}/>}].map((r,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:K.textSoft}}>{r.l}</span>{r.el}</div>)}
            {reverseMode==="income2amount"&&[{l:"הכנסה נטו",el:<AmountInputC value={revIncome} onChange={setRevIncome} style={{width:128}}/>},{l:"PTI מקסימלי %",el:<NI value={revPti} onChange={setRevPti} min={10} max={50} step={1} style={{width:88}}/>},{l:"ריבית %",el:<NI value={revRate} onChange={setRevRate} min={0} max={15} step={0.1} style={{width:88}}/>},{l:"תקופה (שנים)",el:<NI value={revYears} onChange={setRevYears} min={1} max={30} step={1} style={{width:88}}/>}].map((r,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:K.textSoft}}>{r.l}</span>{r.el}</div>)}
            {reverseMode==="price2equity"&&<><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:K.textSoft}}>מחיר דירה</span><AmountInputC value={revPrice} onChange={setRevPrice} style={{width:148}}/></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:K.textSoft}}>סוג עסקה</span><select value={revDealType} onChange={e=>setRevDealType(e.target.value)} style={{...inpS,width:128,cursor:"pointer"}}><option value="first">דירה ראשונה (75%)</option><option value="upgrade">דירה חליפית (70%)</option><option value="invest">השקעה (50%)</option></select></div></>}
          </div>
